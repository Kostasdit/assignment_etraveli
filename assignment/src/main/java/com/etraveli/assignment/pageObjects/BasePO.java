...
import com.etraveli.assignment.utilities.DateUti;
...
public class BasePO {
    ...
    /**
     * wait for element to disappear before going forward
     * @param locator
     */
    ...
    /**
     * press the slider up left->right
     * @param locator
     * @param locatorValue
     * @param value
     */
    public void moveSliderUp(String locator, String locatorValue , Double value) {
        WebElement p = launchDriver.getDriver().findElement(By.cssSelector(locator));
        String result = getText(locatorValue).replaceAll("[^\\d.]", "");
        Double i = Double.parseDouble(result);
        Actions a = new Actions(launchDriver.getDriver()).clickAndHold(p);
        //was take from the web changed a bit
        while(i<value) {
            a.moveByOffset(3,0).perform();
            result = getText(locatorValue).replaceAll("[^\\d.]", "");
    ...
    /**
     * press the slider down right->left
     * @param locator
     * @param locatorValue
     * @param value
     */
    public void moveSliderDown(String locator, String locatorValue , Double value) {
        WebElement p = launchDriver.getDriver().findElement(By.cssSelector(locator));
        String result = getText(locatorValue).replaceAll("[^\\d.]", "");
        Double i = Double.parseDouble(result);
        Actions a = new Actions(launchDriver.getDriver()).clickAndHold(p);
